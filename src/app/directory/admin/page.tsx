  ...
  if (!authed) {
    return (
      <div className="bg-[#0d1b2a] text-[#f4f1ea] min-h-screen flex items-center justify-center px-6">
        ...
        >
          <h1 className="text-xl font-bold mb-4">Directory Admin</h1>
          <input
            type="password"
            value={password}
    ...
    <div className="bg-[#0d1b2a] text-[#f4f1ea] min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          >
            {showAddForm ? "Cancel" : "+ Add Entry Directly"}
          </button>
        </div>
              ...
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                  <span className="inline-block rounded-full bg-[#c9a13b]/20 text-[#c9a13b] text-xs font-medium px-3 py-1 mb-2">
                    {CATEGORY_LABELS[entry.category as DirectoryCategory]}
                  </span>
                  ...
                  <p className="text-xs text-[#f4f1ea]/50 mt-2">
                    Cost: {COST_LABELS[entry.cost_type]}
                    {entry.cost_details ? ` — ${entry.cost_details}` : ""}
                    {entry.city ? ` · ${entry.city}` : ""}
                  </p>
                  {entry.submitted_by_name && (
                    <p className="text-xs text-[#f4f1ea]/50">
...
function AddEntryForm({
  ...
  return (
    ...
    >
      <h3 className="font-semibold">Add a pre-approved entry</h3>
      <input
        required
        placeholder="Name"
        value={form.name}
      ...
      <select
        value={form.category}
        onChange={(e) =>
          setForm({ ...form, category: e.target.value as DirectoryCategory })
        }
        className="w-full rounded-lg bg-white text-[#0d1b2a] px-3 py-2"
      >
      ...
      <div className="grid grid-cols-2 gap-3">
        <select
          ...
          onChange={(e) =>
            setForm({
              ...form,
              cost_type: e.target.value as "free" | "sliding_scale" | "paid",
            })
          }
          className="rounded-lg bg-white text-[#0d1b2a] px-3 py-2"
        >
      ...
      <input
        placeholder="Cost details (optional)"
        value={form.cost_details}
        onChange={(e) => setForm({ ...form, cost_details: e.target.value })}
        className="w-full rounded-lg bg-white text-[#0d1b2a] px-3 py-2"
      />
      <input
